...
public class FileService {
	...
	public void ex1() {
		...
		//리눅스, mac에서는Users/, 윈도우에서는 C:, D:/ 등
		File directory = new File("/io_test/20250305");
		
		//존재하지 않는 폴더를관리하는 File객체 생성
		
		if( !directory.exists() ) {
			directory.mkdirs(); // 폴더를 모두 생성
	...
	public void method2(){
		...
		File file = new File("\\io_test\\20250305\\파일생성.txt"); // 문자열에서 \\를 표기한느 방법 \ 하나는 개행문자
		if(!file.exists()) {
			try { if(file.createNewFile()) System.out.println(file.getName() + "파일이 생성되엇습니다."); } catch(IOException e) {e.printStackTrace();}
			//파일 클래스의 메서느는 대부분 ioEXCEPTION을 발생시킴
			//-> checked exception이므로 예외처리 필수!
	...
	public void method3() {
		//File객체 생성
		File directory = new File("\\homeworkspace\\01_java\\12_IO");
	
	
		//지	정된 경로에 있는 모든 파일/디렉토리를 File[] 형태로 얻어오기
