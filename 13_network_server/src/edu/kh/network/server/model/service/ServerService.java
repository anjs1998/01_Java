...
public class ServerService {
	...
	//4.접속 요청이 오면 요청 수락 후 해당 클라이언트에 대한 소켓 객체 생성
		...
		clientSocket = serverSocket.accept(); // client측 socket 반환
		//-> 클라이언트의 요청이 오고, 수락이될때까지 무한대기
	...
	//6.보조 스트림을 통해 성능 개선
		...
		// 문자 기반 BufferedReader와 바이트 기반 InputStream을 연결할수 있도록
	...
	//7.스트림을 통해 읽고 쓰기
		...
		Date now = new Date(); //java.util.Date : 자바에서 시간을 나타내는 객체
								//new Date() 객체 생성시 현재 시간이 저장되어있다.
		
		//SimpleDateFormat : 날짜 형식을 간단히 지정하는 객체
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일 HH:mm:ss");
		
		String str = sdf.format(now) + "[서버 접속 성공]";
		//PrintWriter.println(문자열) : 주어진 data를 문자 형태로 출력하고, 출력후 줄바꿈 추가
